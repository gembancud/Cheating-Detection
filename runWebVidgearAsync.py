...
async def my_frame_producer():
    ...
    camera_server= NetGear_Async(receive_mode=True, logging=True, port=8001, pattern=2).launch()
    async for frame in camera_server.recv_generator():
        # do something with received frames here
        # if Controller.generatePose:
        frame  =await cd.GeneratePose(frame)
        # if Controller.generatePose and Controller.detectCheat:
        frame = await cd.DetectCheat()
...
async def video_server(scope):
    ...
    # asyncio.set_event_loop(Controller.server.loop)
